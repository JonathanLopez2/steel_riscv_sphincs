        ...
        const spx_ctx *ctx)
{
    asm volatile (
      ...
      "fsw f13, 0(%[address_sw]) \n"
      "fsw f12, 4(%[address_sw]) \n"
      "fsw f9,  8(%[address_sw]) \n"
      "fsw f8,  12(%[address_sw]) \n"
      "fsw f7,  16(%[address_sw]) \n"
      "fsw f6,  20(%[address_sw]) \n"
      "fsw f3,  24(%[address_sw]) \n"
      "fsw f2,  28(%[address_sw]) \n"
      : : [address_sw]"r" (out), [address_lw] "r" (in) : "cc","memory"
  );
}

void haraka512(unsigned char *out, const unsigned char *in, const spx_ctx *ctx)
{
    haraka512_perm(out, in, ctx);
}
        ...
        const spx_ctx *ctx)
{
    asm volatile (
      "flw f8, 0(%[address_lw]) \n"
      "flw f9, 4(%[address_lw]) \n"
      "flw f10, 8(%[address_lw]) \n"
      "flw f11, 12(%[address_lw]) \n"
      "flw f12, 16(%[address_lw]) \n"
      "flw f13, 20(%[address_lw]) \n"
      "flw f14, 24(%[address_lw]) \n"
      "flw f15, 28(%[address_lw]) \n"
      "nop \n"
      ...
      "fsw f15, 0(%[address_sw]) \n"
      "fsw f14, 4(%[address_sw]) \n"
      "fsw f13, 8(%[address_sw]) \n"
      "fsw f12, 12(%[address_sw]) \n"
      "fsw f11, 16(%[address_sw]) \n"
      "fsw f10, 20(%[address_sw]) \n"
      "fsw f9,  24(%[address_sw]) \n"
      "fsw f8,  28(%[address_sw]) \n"
      : : [address_sw]"r" (out), [address_lw] "r" (in) : "cc","memory"
  );
}
