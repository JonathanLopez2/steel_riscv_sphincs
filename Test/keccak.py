...
class Keccak(object):
  ...
  def __repr__(self):
    ...
    for x in range(0, 25, 5):
      lines.append(
        ' '.join('{:016x}'.format(state[x + y]).replace('0', '0')
                 for y in range(5)))
    return '\n'.join(lines)
...
class KeccakHash(object):
  ...
  def absorb(self, b):
    todo = len(b) # Longitud de los datos de entrada
    print("longitud de entrada(todo): ", todo)
    i = 0
    while todo > 0:
      cando = self.rate - self.i
      willabsorb = min(cando, todo) # Aqui evalua si se haran mas bloques del tamaño del rate en este caso no es necesario
      band = np.frombuffer(b[i:i+willabsorb], dtype=np.uint8)
      self.buf[self.i:self.i + willabsorb] ^= np.frombuffer(b[i:i+willabsorb], dtype=np.uint8) # xor con el buffer vacio y hasta la longitud del input
      self.i += willabsorb
      if self.i == self.rate:
        self.permute()
      todo -= willabsorb
  ...
  def squeeze(self, n):
    ...
    while tosqueeze > 0:
      ...
      b += self.k.state.view(dtype=np.uint8)[self.i:self.i + willsqueeze].tobytes()
      self.i += willsqueeze
      if self.i == self.rate:
        self.permute()
  ...
  def pad(self):
    self.buf[self.i] ^= self.dsbyte # Xor del buffer en la longitud del input con 0x1F
    self.buf[self.rate - 1] ^= 0x80 # xor hasta el rate
    print("buffer after second xor:", self.buf.view(dtype=np.uint8))
    self.permute()

  def permute(self):
    self.k.state ^= self.buf.view(dtype=np.uint64)
    print("State after xor buf:", self.k.state.view(dtype=np.uint8))
    self.k.F1600()
    print(repr(self.k))
    self.i = 0
    self.buf[:] = 0
...
class Shake(KeccakHash):
  ...
  def digest(self, n
    ...
    if self.direction == _SPONGE_ABSORBING:
      print("Entrada a la funcion de relleno pad")
      self.pad()
...
k=Keccak()
print("Keccak F-1600 before 24 rounds:", k.state)
k.F1600()

print("Keccak F-1600 after 1 rounds: ",repr(k))

#testSHAKE256 = Shake(b='hola'.encode(), rate=200-(512 // 8), dsbyte=0x1f)
#print("Test SHAKE256: ",testSHAKE256.hexdigest(32))
